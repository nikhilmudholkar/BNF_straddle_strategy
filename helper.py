...
def super_trend(df, f, n):  # df is the dataframe, n is the period, f is the factor; f=3, n=7 are commonly used.
    # Calculation of ATR
    ...
    atr_index = list(df.columns).index('ATR')

    df.iat[n - 1, atr_index] = df['TR'][:n - 1].mean()  # .ix is deprecated from pandas verion- 0.19
